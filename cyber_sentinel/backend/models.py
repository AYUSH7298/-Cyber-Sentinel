...
class RawIntel(Base):
    ...
    content_hash = Column(
        ...
        unique=True,
        index=True,
        nullable=True)  # SHA-256 for O(1) dedup
    # Provenance URL of the raw intel
    source_url = Column(String(512), nullable=True)
    language = Column(
        String(16),
        default="en",
        nullable=True)  # 'en', 'hi', 'hinglish'
    fetched_at = Column(DateTime(timezone=True), default=_utcnow)

    # Relationship
    scam_artifact = relationship(
        "ScamArtifact",
        back_populates="raw_intel",
        uselist=False)
...
class ScamArtifact(Base):
    ...
    # Comma-separated shortened URLs
    shortened_urls = Column(Text, nullable=True)
    ...
    wa_links = Column(Text, nullable=True)
    # Telegram t.me/ links
    tg_links = Column(Text, nullable=True)
    # Detected Aadhaar number patterns
    aadhaar_refs = Column(Text, nullable=True)
    # Detected PAN card patterns
    pan_refs = Column(Text, nullable=True)
    # Detected IFSC codes
    ifsc_codes = Column(Text, nullable=True)
    platform = Column(String(64), nullable=False)

    # --- Geographic Intelligence ---
    # Comma-separated location mentions
    geo_references = Column(Text, nullable=True)
    # Primary Indian state detected
    state = Column(String(64), nullable=True, index=True)
    district = Column(
        String(64),
        nullable=True,
        index=True)  # Primary district detected
    ...
    updated_at = Column(
        DateTime(
            timezone=True),
        default=_utcnow,
        onupdate=_utcnow)  # type: ignore[call-overload]

    # Relationship
    raw_intel = relationship("RawIntel", back_populates="scam_artifact")
    threat_history = relationship(
        "ThreatHistory",
        back_populates="scam_artifact")
...
class SuspectPhoneNumber(Base):
    ...
    # Comma-separated scam types this number was seen in
    scam_types = Column(Text, nullable=True)
    platforms = Column(Text, nullable=True)          # Comma-separated sources
    # Comma-separated campaign IDs
    campaign_ids = Column(Text, nullable=True)
    first_seen = Column(DateTime(timezone=True), default=_utcnow)
    last_seen = Column(
        DateTime(
            timezone=True),
