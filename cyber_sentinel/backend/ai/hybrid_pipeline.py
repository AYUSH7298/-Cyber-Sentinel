...
class HybridPipeline:
    ...
    def __init__(self):
        ...
        # In a full production deployment, this would be augmented with a local
    ...
    def stage1_local_filter(self, text: str) -> bool:
        ...
        """
        # A simple heuristic: if it contains a URL or a Phone number AND
        # suspicious keywords, it's flagged.
        has_suspicious_words = bool(self.suspicious_keywords.search(text))
        has_entities = bool(
            self.phone_pattern.search(text)) or bool(
